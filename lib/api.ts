...
export async function fetchWebsiteRendered(url: string): Promise<RenderedResult> {
    ...
    try {
        ...
        console.log('🧩 Cards extraites:', playwrightCards.length);
        console.log(playwrightCards.slice(0, 3));
        await context.close();
        console.log('Résumé scraping:');
        console.log('HTML size:', html.length);
        console.log('Intercepted JSON:', interceptedJson.length);
        console.log('Broad JSON:', broadJson.length);
        console.log('Cards:', playwrightCards.length);
